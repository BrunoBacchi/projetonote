...
function todoMain() {
  const DEFAULT_OPTION = "Escolha a categoria";
  ...
  function addEntry(event) {
    ...
    if (inputValue.trim().length == 0 || inputValue2.trim().length == 0
      || dateValue.trim().length == 0 || timeValue.trim().length == 0) {
      alert("Invalid data!");
      return;
    }
  ...
  function renderRows(arr) {
    ...
    console.log(arr.slice(itemsPerPage * (currentPage - 1), itemsPerPage * currentPage));
  ...
  function commitEdit(event) {
    ...
    if (todo.trim().length == 0 || category.trim().length == 0
